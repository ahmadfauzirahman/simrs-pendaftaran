<?php

namespace app\models;

use Yii;

/**
 * This is the model class for table "personal_relation".
 *
 * @property int $personal_relation_id
 * @property string|null $relation_name
 * @property int|null $created_user_id
 * @property string|null $created_time
 * @property int|null $modified_user_id
 * @property string|null $modified_time
 *
 * @property MarketingCardExtDet[] $marketingCardExtDets
 * @property PatientContact[] $patientContacts
 * @property ScrtUser $createdUser
 * @property ScrtUser $modifiedUser
 */
class PersonalRelation extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    public static function tableName()
    {
        return 'personal_relation';
    }

    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['created_user_id', 'modified_user_id'], 'integer'],
            [['created_time', 'modified_time'], 'safe'],
            [['relation_name'], 'string', 'max' => 50],
            [['relation_name'], 'unique'],
            [['created_user_id'], 'exist', 'skipOnError' => true, 'targetClass' => ScrtUser::className(), 'targetAttribute' => ['created_user_id' => 'user_id']],
            [['modified_user_id'], 'exist', 'skipOnError' => true, 'targetClass' => ScrtUser::className(), 'targetAttribute' => ['modified_user_id' => 'user_id']],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function attributeLabels()
    {
        return [
            'personal_relation_id' => 'Personal Relation ID',
            'relation_name' => 'Relation Name',
            'created_user_id' => 'Created User ID',
            'created_time' => 'Created Time',
            'modified_user_id' => 'Modified User ID',
            'modified_time' => 'Modified Time',
        ];
    }

    /**
     * Gets query for [[MarketingCardExtDets]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getMarketingCardExtDets()
    {
        return $this->hasMany(MarketingCardExtDet::className(), ['personal_relation_id' => 'personal_relation_id']);
    }

    /**
     * Gets query for [[PatientContacts]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getPatientContacts()
    {
        return $this->hasMany(PatientContact::className(), ['personal_relation_id' => 'personal_relation_id']);
    }

    /**
     * Gets query for [[CreatedUser]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getCreatedUser()
    {
        return $this->hasOne(ScrtUser::className(), ['user_id' => 'created_user_id']);
    }

    /**
     * Gets query for [[ModifiedUser]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getModifiedUser()
    {
        return $this->hasOne(ScrtUser::className(), ['user_id' => 'modified_user_id']);
    }
}
